...
func TestMap_OmitEmpty(t *testing.T) {
	...
	_, ok = m["AcceptAt"].(map[string]interface{})
	if ok {
		t.Error("Map should not contain the AcceptAt field that is tagged as omitempty")
	}
}
...
func TestMap_OmitNested(t *testing.T) {
	...
	// should not happen
	if _, ok := in["AcceptAt"].(map[string]interface{}); ok {
		t.Error("Map nested struct should omit recursiving parsing of AcceptAt")
	}

	if _, ok := in["AcceptAt"].(time.Time); !ok {
		t.Error("Map nested struct should stop parsing of AcceptAt at is current value")
	}
}
...
func TestMap_TimeField(t *testing.T) {
	...
	if !ok {
		t.Error("AcceptAt field must be final")
	}
}
